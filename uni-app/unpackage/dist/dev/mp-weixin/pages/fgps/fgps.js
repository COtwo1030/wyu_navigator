  ...
  methods: {
    ...
    resetPassword() {
      if (!this.email || !this.code || !this.password || !this.confirm_password) {
        return common_vendor.index.showToast({ title: "请完善信息", icon: "none" });
      }
      ...
      common_vendor.index.request({
        ...
        success: (res) => {
          if (res.statusCode === 200) {
            ...
            common_vendor.index.showToast({ title: res.data.detail || "重置失败", icon: "none" });
          }
        },
        complete: () => {
          this.loading = false;
...
function _sfc_render(_ctx, _cache, $props, $setup, $data, $options) {
  return {
    ...
    o: common_vendor.o(($event) => $data.active = "pass"),
    p: common_vendor.o(($event) => $data.active = ""),
    q: $data.password,
    r: common_vendor.o(($event) => $data.password = $event.detail.value),
    s: $data.active === "pass" ? 1 : "",
    t: common_vendor.o(($event) => $data.active = "confirm"),
    v: common_vendor.o(($event) => $data.active = ""),
    w: $data.confirm_password,
    x: common_vendor.o(($event) => $data.confirm_password = $event.detail.value),
    y: $data.active === "confirm" ? 1 : "",
    z: $data.loading,
...
//# sourceMappingURL=../../../.sourcemap/mp-weixin/pages/fgps/fgps.js.map
